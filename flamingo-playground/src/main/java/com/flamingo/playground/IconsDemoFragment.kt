...
class IconsDemoFragment : Fragment() {
    ...
    private var icons by mutableStateOf<List<Icon>?>(null)
    ...
    private fun LoadIcons(context: Context) = LaunchedEffect(Unit) {
        launch(Dispatchers.IO) {
            icons = loadIcons().map {
                val icon = Flamingo.icons.fromId(it)
                Icon(
    ...
    @Composable
    private fun IconBox(icon: Icon) = Column(
        modifier = Modifier
            .aspectRatio(1f)
        ...
        Icon(icon = icon.icon)
        Text(text = icon.name, textAlign = TextAlign.Center, style = Flamingo.typography.body2)
    }

    companion object {
        /**
         * Reflection is used because the [designSystemIcons] array field cannot be made public (it
